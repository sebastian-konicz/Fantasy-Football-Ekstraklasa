...
def main():
    ...
    links_df = pd.read_csv(path + r'\data\raw\ekstraclass\01_players_links_{date}.csv'.format(date=var.time_stamp), delimiter=';')
    # links_df = pd.read_csv(path + r'\data\raw\ekstraclass\01_players_links_short.csv', delimiter=';')
    ...
    for link in link_list:
        ...
        data_tuples = list(zip(col_id, col_name, col_position, col_value, col_club, col_club_abr, col_club_prev,
                               col_country, col_season, col_time, col_goals, col_assists, col_own_goal, col_penalty,
        ...
        player_stats = pd.DataFrame(data_tuples, columns=["id", "name", "position", "value", "club", "club_abr",
                                                          "club_prev", "country", "season", "time", "goals", "assists",
                                                          "own_goal", "penalty", "penalty_won", "penalty_given",
                                                          "penalty_lost", "penalty_defended", "in_stat", "yellow_card",
                                                          "red_card", "points"])
    ...
    # getting rid of current season stats (season = SUMA)
    players_stats = players_stats.drop(players_stats[players_stats['season'] == 'SUMA'].index)
